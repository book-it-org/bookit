...
<template>
    ...
    <AppLayout header-limpo>
        <div class="container mx-auto flex h-full flex-1 flex-col gap-4 rounded-xl">
            <div v-if="carrinho.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <CardOfertaCarrinho
            ...
            <Card v-if="carrinho.length > 0" class="items-center p-6">
                <div class="grid w-full max-w-md grid-cols-2 gap-4 text-sm">
                    ...
                    <p class="text-end font-semibold">R$ {{ total.toFixed(2).replace('.', ',') }}</p>
                    <p class="font-medium">Frete:</p>
                    <p class="text-end font-semibold">R$ {{ frete.toFixed(2).replace('.', ',') }}</p>
                    <div class="col-span-2 border-t pt-2 mt-2">
                        <div class="grid grid-cols-2 gap-4">
                            <p class="font-bold text-lg">Total:</p>
                            <p class="text-end font-bold text-lg text-emerald-600">
                                R$ {{ totalComFrete.toFixed(2).replace('.', ',') }}
                            </p>
                        </div>
                    </div>
                </div>
                <Button class="w-full mt-4 md:w-auto" size="lg">
                    Finalizar compra
                </Button>
            </Card>
        </div>
